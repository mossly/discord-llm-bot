...
class ReminderToolV2(BaseTool):
    ...
    def parameters(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "timestamp": {
                    "type": "number",
                    "description": "Unix timestamp as fallback for 'set' action if natural language parsing fails. Calculate from current time + desired delay (optional)."
                },
                "reminder_timestamp": {
                    "type": "number",
    ...
    async def execute(self, action: str, user_id: str, reminder_text: str = None, 
                     time_str: str = None, timestamp: float = None, reminder_timestamp: float = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "set":
                return await self._set_reminder(user_id, reminder_text, time_str, timestamp)
            elif action == "list":
    ...
    async def _set_reminder(self, user_id: int, reminder_text: str, time_str: str = None, timestamp: float = None) -> Dict[str, Any]:
        """Set a new reminder with dual input support"""
        ...
        # Must have either time_str or timestamp
        if not time_str and not timestamp:
            return {
                "success": False,
                "error": "Either natural language time or timestamp is required"
            }
        ...
        # Try natural language parsing first (preferred)
        if time_str:
            ...
            if target_dt:
                logger.info(f"Successfully parsed '{time_str}' as {target_dt} ({target_dt.tzinfo})")
        ...
        # Fallback to timestamp if natural language parsing failed
        if trigger_time is None and timestamp:
            logger.info(f"Using fallback timestamp {timestamp} for user {user_id}")
            trigger_time = timestamp
            # Convert timestamp to datetime for display purposes
            target_dt = datetime.utcfromtimestamp(timestamp).replace(tzinfo=pytz.UTC).astimezone(pytz.timezone(user_timezone))
        
        # If both methods failed, return helpful error
